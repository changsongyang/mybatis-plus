...
public final class SqlHelper {
     ...
     *
     * @param entityClass 实体类
     * @param function    lambda操作，例如 {@code m->m.selectList(wrapper)}
     * @param <T>         实体类的类型
     * @param <R>         返回值类型
     * @param <M>         Mapper类型
     * @return 返回lambda执行结果
     */
    public static <T, R, M extends BaseMapper<T>> R execute(Class<T> entityClass, Function<M, R> function) {
        SqlSession sqlSession = SqlHelper.sqlSession(entityClass);
        try {
            return function.apply(SqlHelper.getMapper(entityClass, sqlSession));
        } finally {
