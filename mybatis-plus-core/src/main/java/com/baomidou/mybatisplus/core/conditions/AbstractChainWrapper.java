...
public abstract class AbstractChainWrapper<T, Children extends AbstractChainWrapper<T, Children, Param>,
    ...
    public String convSf2ColInSel(SFunction<T, ?> function) {
        throw ExceptionUtils.mpe("can not use this method for \"%s\"", "convSf2ColInSel");
    }

    @Override
    public String convSf2Col(SFunction<T, ?> mutable) {
        throw ExceptionUtils.mpe("can not use this method for \"%s\"", "convSf2Col");
    }

    @Override
    public String convSf2ColMapping(SFunction<T, ?> function) {
        throw ExceptionUtils.mpe("can not use this method for \"%s\"", "convSf2ColMapping");
    }

    @Override
    public String checkStrCol(String column) {
        throw ExceptionUtils.mpe("can not use this method for \"%s\"", "checkStrCol");
    }
    ...
    public <R> R execute(Function<BaseMapper<T>, R> function) {
        BaseMapper<T> targetMapper = baseMapper;
        if (targetMapper == null) {
            targetMapper = CompatibleHelper.getCompatibleSet().getContextBaseMapper(delegateWrapper.getContext().getEntityClass());
        }
        return function.apply(targetMapper);
    }
}
