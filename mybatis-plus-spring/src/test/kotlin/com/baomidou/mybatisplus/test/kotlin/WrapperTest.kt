...
class WrapperTest {
    ...
    fun testLambdaQuery() {
        logSqlSegment(
            "测试1.1 LambdaKt", QueryWrapper(User()).eq(User::name, "sss").eq(User::roleId, "sss2")
            .apply("a=1").apply("b={0}", 3).apply("c={0} and d={1}", 4, 5))
    }

    @Test
    fun testLambdaUpdate() {
        logSqlSegmentAndSetSql(
            "测试2.1 LambdaKt", UpdateWrapper(User()).eq(User::name, "sss").eq(User::roleId, "sss2")
