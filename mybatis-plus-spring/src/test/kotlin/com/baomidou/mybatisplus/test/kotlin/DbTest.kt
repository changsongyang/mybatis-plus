...
class DbTest : BaseDbTest<UserMapper>() {
    ...
    fun testUpdate() {
        ...
        assertThat("sheep").isEqualTo(Db.ktQuery(User::class.java).eq(User::id, 1).one().name)
    ...
    @Test
    fun remove() {
        val isSuccessId = Db.remove(Db.ktQuery(User::class.java).eq(User::id, 1).delegate())
        Assertions.assertTrue(isSuccessId)
        val isSuccessName = Db.remove(Db.ktQuery(User::class.java).eq(User::name, "gozei").delegate())
